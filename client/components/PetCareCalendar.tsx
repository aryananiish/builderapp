    ...
    <div className={cn("space-y-4 md:space-y-6", className)}>
      ...
      <Card className="p-4 md:p-6 bg-card border-border overflow-hidden">
        <div className="mb-4 md:mb-6">
          <h2 className="text-lg md:text-xl font-semibold text-foreground mb-2">Weekly Pet Care Schedule</h2>
          <p className="text-sm md:text-base text-muted-foreground">Keep track of vaccines, feeding times, and appointments</p>
        </div>
              ...
              {timeSlots.map((time) => (
                <div key={time} className="grid grid-cols-8 gap-2 min-h-[60px]">
                  ...
                  {days.map((day) => {
                    ...
                    const eventAtTime = dayEvents.find(event => event.time === time);
                              ...
                              <div className="mt-auto space-y-1">
                                {eventAtTime.location && (
                                  <div className="flex items-center space-x-1">
                                    ...
                                    <span className="text-xs opacity-60 line-clamp-1">{eventAtTime.location}</span>
                                  </div>
                                )}
                                <Badge 
                                  variant="outline" 
                                  className={cn("text-xs", getStatusColor(eventAtTime.status))}
                                >
                                  {eventAtTime.status}
                                </Badge>
                              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="flex items-center space-x-3">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-foreground font-medium">Vaccines</span>
            </div>
            <span className="text-xs text-muted-foreground">Health & Prevention</span>
          </div>
          <div className="flex items-center space-x-3">
            <div className="flex items-center space-x-2">
              <Utensils className="h-4 w-4 text-blue-400" />
              <span className="text-sm text-foreground font-medium">Feeding</span>
            </div>
            <span className="text-xs text-muted-foreground">Meals & Nutrition</span>
          </div>
          <div className="flex items-center space-x-3">
            <div className="flex items-center space-x-2">
            ...
            <span className="text-xs text-muted-foreground">Grooming & Checkups</span>
          </div>
        </div>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-4 bg-card border-border">
          <div className="text-center">
            ...
            <div className="text-2xl font-bold text-primary">{summary?.upcomingVaccines || 0}</div>
        ...
        <Card className="p-4 bg-card border-border">
          <div className="text-center">
            ...
            <div className="text-2xl font-bold text-success">{summary?.dailyMeals || 0}</div>
        ...
        <Card className="p-4 bg-card border-border">
          <div className="text-center">
            ...
            <div className="text-2xl font-bold text-info">{summary?.totalPets || 0}</div>
        ...
        <Card className="p-4 bg-card border-border">
          <div className="text-center">
            ...
            <div className="text-2xl font-bold text-warning">{summary?.upcomingEvents || 0}</div>
